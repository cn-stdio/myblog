...
var articleId = htmlUrl.substring(htmlUrl.lastIndexOf("/")+1, htmlUrl.length);
var tagCount = 0;
$.ajax(
    {
        type:"post",
        url:"/getArticleEditContent",
        dataType:"json",
        async: false,
        data:{
            articleId: articleId
        ...
        success:function(data){
            if(data['msg']=="noLogin") {
                ...
                $("#write-title-text").val(data['title']);
                $(".editormd-markdown-textarea").val(data['content']);
                $.each(data['tags'], function(index, obj){
                    var tagUnitOne = '<div style="display: inline-block;">\n' +
                        '                        <p class="tag-name" contenteditable="true">' + obj + '</p>\n' +
                        '                        <i class="am-icon-times remove-tag" style="color: #CCCCCC; cursor: pointer;"></i>\n' +
                        '                    </div>';

                    $("#publish-modal-tags").append(tagUnitOne);
                    tagCount ++;
                });
                ...
                $("#publish-modal-classify").val(data['classify']);
                $("#publish-modal-type").val(data['type']);
            }
        },
        error:function(){
            console.log("文章显示失败！");
